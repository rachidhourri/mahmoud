...
?>			
	...
	<?php if ($btnlink != '') { ?>
		<a href="<?php echo esc_url($btnlink); ?>" class="btn btn-line-black btn-big <?php echo vc_shortcode_custom_css_class( $css ); ?>"><?php echo htmlspecialchars_decode($btntext); ?></a>
	<?php } ?>
...
?>			
	<?php if ( strlen( $linkbox ) > 0 && strlen( $url['url'] ) > 0 ) {
		echo '<a href="' . esc_attr( $url['url'] ) . '" title="' . esc_attr( $url['title'] ) . '" target="' . ( strlen( $url['target'] ) > 0 ? esc_attr( $url['target'] ) : '_self' ) . '" class="btn ' . $class . vc_shortcode_custom_css_class( $css ) . '">' . esc_attr( $url['title'] ) . '</a>';
	} ?>
...
function process_func($atts, $content = null){
    ...
    <div class="de_tab tab_steps tab_<?php echo esc_attr($number1); ?>">
        <ul class="de_nav">
	        <?php
	        	$i=0;
	        	if ($idcate_process != '') {
...
function foliof_func($atts, $content = null){
    ...
    <div id="gallery" class="<?php echo esc_attr($extraclass); ?> gallery full-gallery de-gallery pf_full_width <?php if ($columns == 2) {echo 'pf_2_cols'; }elseif ($columns == 3) { echo 'pf_3_cols'; }elseif ($columns == 5) { echo 'pf_5_cols'; }elseif ($columns == 6) { echo 'pf_6_cols'; }else{} ?> wow fadeInUp" data-wow-delay=".3s" style="margin:0px <?php echo esc_attr($gap1); ?>" >
        <?php 
...
function cate_portfolio_func($atts, $content = null){
    ...
    <div id="<?php echo esc_attr($portfolio_category_isotope); ?>" class="gallery full-gallery de-gallery pf_full_width portfolio_category <?php if ($columns == 2) {echo 'pf_2_cols'; }elseif ($columns == 3) { echo 'pf_3_cols'; }elseif ($columns == 5) { echo 'pf_5_cols'; }elseif ($columns == 6) { echo 'pf_6_cols'; }else{} ?> wow fadeInUp <?php echo vc_shortcode_custom_css_class( $css ); ?>" data-wow-delay=".3s" style="margin: -<?php echo esc_attr($gap1); ?> <?php echo esc_attr($gap1); ?>" >
        ...
        <div class="item <?php echo esc_attr($cate_slug) . esc_attr($item_isotope); ?>">
            <div class="picframe" style="margin:<?php echo esc_attr($gap1); ?>">
                <a <?php global $archi_option; if($archi_option['ajax_work']!=false){ ?>class="<?php if($arrow_btn == 'no'){echo 'single-ajax-popup';}else{echo 'ajax-popup-' . esc_attr($cate_slug);} ?>"<?php } ?> href="<?php the_permalink(); ?>">
                    <span class="overlay">
                        <span class="pf_text">
                            <span class="project-name"><?php the_title(); ?></span>
                        </span>
